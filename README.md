...
`STR_IN`（LOW=自動モード有効）と`ENGOFF_IN`（LOW=運転許可）の組み合わせで制御。

### 始動シーケンス（スタータモータ保護付き）

```text
[条件] speed < 20.0km/h かつ ENG_ON=false かつ 失敗・休止中でない

  ┌─ サイクル開始 (crankCycles++) ─────────────────────────────────┐
  │  STR_OUT LOW（スタータON）                                     │
  │  ├─ 1500rpm × 0.1s継続 → 始動成功: STR_OUT HIGH, crankCycles=0 │
  │  └─ 2秒経過（未始動）  → STR_OUT HIGH, 1秒休止                │
  │              └─ 3サイクル未満 → 再始動（次サイクルへ）         │
  │              └─ 3サイクル到達 → crankFailed=true（要手動リセット）│
  └────────────────────────────────────────────────────────────────┘
```

| 条件 | 動作 |
| ---- | ---- |
| speed < 20.0km/h | エンジン始動（スタータON） |
| 1500rpm × 0.1s継続 | 始動完了・スタータOFF |
| 2秒連続ONで未始動 | スタータOFF → 1秒休止 → 再試行 |
| 3サイクル失敗 | `crankFailed=true`（キルスイッチ操作でリセット） |
| speed >= 30.0km/h | エンジン停止・全フラグリセット |
| `ENGOFF_IN` HIGH / `STR_IN` HIGH | エンジン停止・全フラグリセット |

## 拡張アイデア (TODO)

- [x] MAP内パラメータ選択・燃料噴射・点火処理高速化  
  (現状では処理遅れに起因すると思われる過大な進角角度を設定している)
- [x] 自動エンジン始動・停止（燃費追求モード）実装  
  - 20km/h以下で始動、30km/h以上で停止  
  - スタータモータ保護：2秒ON→1秒休止を最大3サイクル
