...
unsigned long starterFirstMs  = 0;        // クランキング開始時刻 (ms)
bool          starterActive   = false;    // クランキング継続中フラグ
bool          useStartMap     = false;    // 始動MAP使用フラグ（startState==LOW の代替）
...
void cycleReset() {
  ...
  int16_t inj_end_ca = useStartMap ? start_INJ_END_CA : INJ_END_CA;
    ...
    if (fastestdigitalRead(STR_IN) == LOW) {          // スタートスイッチON
      if (fastestdigitalRead(ENGOFF_IN) == LOW) {     // キルスイッチON（運転許可）
        if (starterState == STR_IDLE || starterState == STR_FAILED) {
          ...
          starterActive   = false;
          starterFirstMs  = 0;
          useStartMap     = true;   // cycleReset() より先にセット→始動MAP適用
          ENG_ON          = true;
          Launch          = true;
      ...
      case STR_CRANKING: {                            // クランキング中
        ...
        if (tachoRpm >= start_RPM) {
          if (!starterActive) {                           // 始動成功判定開始
            ...
            fastestdigitalWrite(STR_OUT, HIGH);             // スタータOFF
            STR_IN_state = false;                           // スタータ入力状態OFF
            useStartMap  = false;                           // 始動MAP使用フラグOFF
            break;  // タイムアウト確認をスキップ
          }
