...
#define AUTO_CRANK_MAX_ON_MS  2000UL  // スタータモータ最大連続ON時間（ms）
#define AUTO_CRANK_REST_MS    1000UL  // スタータモータ休止時間（ms）
#define AUTO_CRANK_MAX_CYCLES 3       // スタータモータ最大試行サイクル数
...
unsigned long crankOnTime      = 0;       // スタータON開始時刻（ms）
unsigned long crankRestTime    = 0;       // スタータ休止開始時刻（ms）
uint8_t       crankCycles      = 0;       // スタータ試行サイクル数
bool          crankResting     = false;   // スタータ休止中フラグ
bool          crankFailed      = false;   // スタータ始動失敗フラグ（3サイクル超過）
...
void Routine() {
  ...
  if (fastestdigitalRead(ENGOFF_IN) == LOW) {   // ハードキルスイッチが運転許可状態
    if (startState == LOW) {                    // 自動モード有効
      // 20km/hを下回ったらエンジン始動（停止中かつ失敗・休止中でない場合のみ）
      if (!ENG_ON && !cranking && !crankResting && !crankFailed && speed < AUTO_START_SPEED) {
        ENG_ON = true;
        cranking = true;
        Launch = true;
        STR_IN_state = true;
      ...
      // スタータモータ保護：2秒連続ON後に休止
      if (cranking) {
        if (millis() - crankOnTime >= AUTO_CRANK_MAX_ON_MS) {
          // 2秒経過：スタータOFF → 休止開始
          cranking = false;
          ENG_ON = false;
          STR_IN_state = false;
          ...
          rpm1500StartTime = 0;
          crankResting = true;
          crankRestTime = millis();
        } else if (tachoRpm >= AUTO_CRANK_END_RPM) {
          ...
          if (rpm1500StartTime == 0) {
            ...
            STR_IN_state = false;
            fastestdigitalWrite(STR_OUT, HIGH); // スタータモータOFF
            crankCycles = 0;                    // 始動成功：サイクルカウントリセット
          }
        } else {
          rpm1500StartTime = 0;                 // 1500rpm未満なら継続時間リセット
        }
      }
      // 休止中の処理：1秒経過後に再始動、または3サイクル超過で失敗確定
      if (crankResting) {
        if (millis() - crankRestTime >= AUTO_CRANK_REST_MS) {
          crankResting = false;
          if (crankCycles >= AUTO_CRANK_MAX_CYCLES) {
            // 3サイクル完了しても始動せず：失敗確定（キルスイッチ操作でリセット）
            crankFailed = true;
          } else {
            // 再始動
            ENG_ON = true;
